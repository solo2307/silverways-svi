...
class GoogleStaticStore:
    def __init__(
        ...
        cache_folder: Path | str = "cache/google-streetview",
        *,
...
def process_pano_csv(
    ...
    *,
    ...
    if output_metadata_csv:
        if not Path(output_metadata_csv).exists():
            generate_list_of_downloaded_files(output_dir, df, output_metadata_csv)
        seen_pairs = _load_manifest_existing_pairs(Path(output_metadata_csv))

    # if not output_metadata_csv.exists():
    #     generate_list_of_downloaded_files(output_dir, df, output_metadata_csv)
    # seen_pairs = (
    #     _load_manifest_existing_pairs(output_metadata_csv)
    #     if output_metadata_csv
    #     else set()
    # )
    # Prepare manifest for appends
    write_header = output_metadata_csv and (
        not Path(output_metadata_csv).exists()
        or Path(output_metadata_csv).stat().st_size == 0
    )
    manifest_f = None
    manifest_writer = None
    if output_metadata_csv:
        Path(output_metadata_csv).parent.mkdir(parents=True, exist_ok=True)
        manifest_f = Path(output_metadata_csv).open("a", newline="", encoding="utf-8")
    ...
    store = GoogleStaticStore(
        api_key=api_key, cache_folder=output_dir, fov=fov, pitch=pitch, size=size
    )
    ...
    try:
        for idx, row in tqdm(df.iterrows(), total=len(df), desc="Processing panoramas"):
            ...
            for heading in headings:
                ...
                if manifest_writer and (pano_id, int(heading)) in seen_pairs:
                    saved_count += 1
                    continue
                image_path, _ = store.fetch_image_with_pano(
                    pano_id, heading=int(heading)
                )
            ...
            if max_requests is not None and request_count >= max_requests:
                logger.info(
                    "Reached max_requests, send requests=%d; stopping. Saved before = %d",
                    request_count,
                    saved_count,
                )
        ...
        if manifest_f:
            manifest_f.close()
    logger.info(
        "Images present/downloaded: %d/%d → %s", saved_count, request_count, output_dir
    )
    return request_count
...
def run_pipeline(cfg: DictConfig) -> int:
    try:
        ...
        output_img_dir = sds_dir / _select(
            cfg, "datasets.streetview.svi_dir", "google-streetview"
        )  # 'cache/google-streetview'
        panorama_metadata_csv = (
            Path(cfg.storage.cache) / cfg.datasets.streetview.output_panorama_metadata
        )  #'cache/mannheim_google_panorama_metadata.csv'
        ...
        headings = _select(
            cfg, "datasets.streetview.images.headings", [0, 90, 180, 270]
        )
