...
def sanitize_columns(gdf):
    ...
    gdf.columns = gdf.columns.str.replace(":", "_", regex=False).str.replace(
        "@", "", regex=False
    )
...
def run_pipeline(cfg: DictConfig) -> int:
    try:
        ...
        if cfg.datasets.osm.buildings in ["True", "true", True]:
            _ = client.get_buildings(
                bbox=True,
                output_path=Path(cfg.storage.sds)
                / cfg.datasets.osm.dir
                / "buildings.gpkg",
            )
        return 0
    except Exception:
