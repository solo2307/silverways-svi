...
class SketchMapToolClient:
    def __init__(
        self,
        base_url="https://test.sketch-map-tool.heigit.org",
        factor=0.227,
        image_size=(1716, 1436),
    ):
...
def generate_and_download_sketches_roi(
    city_name, roi_path, csv_path, tile_width, grid_path, download_dir
):
    ...
    if not os.path.exists(csv_path):
        ...
        print(
            f"📄 Loaded existing submissions from {csv_path} ({len(submitted_df)} entries)"
        )
    ...
    process_cnt = 0
    for idx, row in tqdm(
        submitted_df.iterrows(), total=len(submitted_df), desc="Processing submissions"
    ):
        uuid = row["uuid"]
        # fid = row["fid"]
        bbox = row["bbox"]
    ...
    if resave:
        ...
        merged_df = grid_df.merge(
            submitted_df[["geohash", "uuid"]], on="geohash", how="left"
        )
...
if __name__ == "__main__":
    ...
    generate_and_download_sketches_roi(
        city_name, roi_path, csv_path, tile_width, grid_path, download_dir
    )
