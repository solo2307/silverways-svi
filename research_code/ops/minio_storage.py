...
def upload_folder(
    client, bucket_name, local_folder, remote_folder, allowed_extensions=None
):
    for root, dirs, files in os.walk(local_folder):
        ...
        for file_name in tqdm(
            files, total=len(files), desc=f"Uploading to {remote_folder}"
        ):
            if allowed_extensions and not file_name.lower().endswith(
...
def download_from_minio(
    client: Minio, bucket_name: str, remote_prefix: str, local_target_dir: str
):
    """
...
def main_back_up_images(cfg: DictConfig):
    ...
    allowed_extensions = [
        ...
        ".png",
        ".tiff",
    ]  # Specify desired file formats here
    client = Minio(
    ...
    try:
        ...
        upload_folder(
            client, bucket_name, local_folder, remote_folder, allowed_extensions
        )
    except S3Error as err:
        print("S3 Error:", err)
