...
def select_xyz_roi(
    files: List[str], roi_file: Optional[str] = None, tile_file: Optional[str] = None
) -> List[str]:
    selected_files = []
    ...
    if roi_file is None and tile_file is not None:
        ...
        tile_ids = [
            f"dgm1_32_{dop[2:5]}_{dop[-4:]}_1_bw_2022.xyz" for dop in dopkachel_ids
        ]
...
def generate_dem(output_tif="cache/mannheim_dem.tif"):
    ...
    selected_files = select_xyz_roi(
        tile_file="data/opengeodata_mannheim.gpkg", files=files
    )
    ...
    if len(selected_files) == 0:
        ...
        geotiffs = []
        for f in tqdm(
            selected_files, desc="Converting XYZ to GeoTIFF", total=len(selected_files)
        ):
