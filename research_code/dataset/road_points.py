...
def get_panorama_metadata(lat: float, lon: float, api_key: str, session: requests.Session) -> dict:
    ...
    try:
        ...
        if status == "OK":
            ...
            return {"pano_lat": loc.get("lat"), "pano_lon": loc.get("lng"),
                    "pano_id": data.get("pano_id"), "date": data.get("date"), "status": status}
        return {"pano_lat": None, "pano_lon": None, "pano_id": None, "date": None, "status": status}
    except Exception as e:
...
def retreat_panorama_metadata(gdf_points: gpd.GeoDataFrame, api_key: str, output_csv: str,
    ...
    output_path = Path(output_csv); output_path.parent.mkdir(parents=True, exist_ok=True)

    fieldnames = ["uuid","lat","lon","osm_id","pano_lat","pano_lon","pano_id","date","status"]
    existing = set()
    if output_path.exists() and output_path.stat().st_size > 0:
        try:
            for r in pd.read_csv(output_path, usecols=["lat","lon"]).itertuples(index=False):
    ...
    with output_path.open("a", newline="", encoding="utf-8") as f:
        ...
        for row in tqdm(gdf_wgs.itertuples(), total=len(gdf_wgs), desc="📷 Fetching pano metadata"):
            ...
            w.writerow({
                "uuid": getattr(row, "uuid", str(uuid.uuid4())),
                "lat": lat, "lon": lon, "osm_id": getattr(row, "osm_id", None),
                "pano_lat": meta["pano_lat"], "pano_lon": meta["pano_lon"],
                "pano_id": meta["pano_id"], "date": meta["date"], "status": meta["status"]
            })
            time.sleep(delay)
...
class RoadDataset:
    ...
    def generate_points(self, point_step: int = 100, merge_distance:int = 20) -> gpd.GeoDataFrame:
...
def run_pipeline(cfg: DictConfig):
    ...
    try:
        ...
        out_csv = Path(cfg.storage.cache) / cfg.datasets.streetview.output_panorama_metadata
        ...
        if not out_points.exists():
            ...
            gdf_points = roads.generate_points(point_step=point_step, merge_distance=merge_dist)
        ...
        retreat_panorama_metadata(
            gdf_points.iloc[:2], api_key, str(out_csv),
            delay=delay,
