...
# ADE20K class ids used by this PSPNet setup.
CLASS_IDS = {
    "building": 1,
    "sky": 2,
    "tree": 4,
...
def resolve_device(device: str) -> torch.device:
    ...
    # CPU is safer than MPS for the older mit-semseg stack.
...
class PSPNetPredictor:
    def __init__(self, model_dir: Path, device: str = "auto") -> None:
        ...
        encoder_path = model_dir / "encoder_epoch_50.pth"
        decoder_path = model_dir / "decoder_epoch_50.pth"
    ...
    def predict(self, image: Image.Image) -> np.ndarray:
        ...
        mask = torch.argmax(scores, dim=1)[0]
...
def make_overlay(image: Image.Image, color_mask: np.ndarray, alpha: float = 0.55) -> np.ndarray:
...
def main() -> None:
    ...
    predictor = PSPNetPredictor(model_dir=model_dir, device=args.device)
